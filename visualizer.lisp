...
(defun paint-device (device painter)
  (q+:draw-text painter 0 20 (format NIL "ID: ~a (~a) ~a"
                                    (cl-gamepad:device-id device)
                                    (cl-gamepad:device-vendor device)
                                    (cl-gamepad:device-description device)))
  (q+:translate painter 20 20)
  (q+:draw-text painter 0 *field-height* (format NIL "Axis:"))
  (loop for i from 0 below (cl-gamepad:device-axes device)
        for col = (mod i 10)
        ...
        do (when (= 0 col)
           ...
           (with-translation-saved (painter)
             ...
             (paint-axis (princ-to-string i) (cl-gamepad:device-axis device i) painter)))
  (q+:translate painter 0 (+ *field-height* *field-padding*))
  (q+:draw-text painter 0 *field-height* (format NIL "Buttons:"))
  (loop for i from 0 below (cl-gamepad:device-buttons device)
        for col = (mod i 10)
        for x = (* col (+ *field-width* *field-padding*))
        do (when (= 0 col)
             (q+:translate painter 0 (+ *field-height* *field-padding*)))
           (with-translation-saved (painter)
             (q+:translate painter x 0)
             (paint-button (princ-to-string i) (cl-gamepad:device-button device i) painter))))
