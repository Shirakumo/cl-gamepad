...
(defun query-labels (labels device map &optional (confirm (lambda (e) (typep e 'button-up))))
  (when (< 0 (length labels))
    (loop with i = 0
          ...
          do (out "Press <~a>~@[ (~a)~]~@[ ~a~] ~60t(<A> to skip, <B> to undo)"
             ...
             (cond ((< i 0) (setf i 0))
                   ((= i (length labels))
                    (out "Everything in order? ~60t(<A> to confirm, <B> to undo)")
                    (flet ((process (event)
                             (case (event-label event)
                               (:A (loop-finish))
                               (:B (decf i)))))
...
(defun configure-device (device &key (button-labels +common-buttons+ b-p)
                                     (axis-labels +common-axes+ a-p)
                                     ignored-axes
                                     (mappings-file *default-mappings-file*))
  (let* ((device (ensure-device device T))
    ...
    (flet ((process (event)
      ...
      (out "-> Mapping axes.~%")
      (query-labels axis-labels device axis-map #'process)

      (unless (or a-p b-p)
        (out "-> Are there extra buttons or axes on the device? ~60t(<A> to map, <B> to skip)")
        (when (loop do (flet ((process (event)
                                (when (typep event 'button-up)
                                  (case (event-label event)
                                    (:A (return T))
                                    (:B (return NIL))))))
                         (poll-events device #'process)))
          (out "-> Mapping extra buttons.~%")
          (query-labels +common-extra-buttons+ device button-map)
          
          (out "-> Mapping extra axes.~%")
          (query-labels +common-extra-axes+ device axis-map #'process))))
    ...
    (loop (let ((int (1- (or (parse-integer (read-line *query-io*) :junk-allowed T)
                             (1+ (icon-type device))))))
            (when int
